...
public class JdbcUserDaoTest extends BaseDaoTest {
    ...
    private static final User ADMIN = new User(4, "admin", "admin", "ROLE_ADMIN");
    ...
    @Test
    public void getUserById_given_invalid_id_returns_null() {
        User user = dao.getUserById(-1);
        assertNull(user, "getUserById with invalid userId did not return null user");
    }

    @Test
    public void getUserById_given_valid_id_returns_user() {
    ...
    public void findAll_returns_all_users() {
        ...
        assertEquals(4, users.size(), "getUsers returned a list with the incorrect number of users");
        assertEquals(ADMIN, users.get(0), "getUsers returned a list in incorrect order");
        assertEquals(USER_1, users.get(1), "getUsers returned a list in incorrect order");
        assertEquals(USER_2, users.get(2), "getUsers returned a list in incorrect order");
        assertEquals(USER_3, users.get(3), "getUsers returned a list in incorrect order");
    }
